...
* Detail: more direct manipulation
:PROPERTIES:
:CUSTOM_ID: interaction-manipulation-2
:END:

Dragging the circle center around is nicer than setting the two sliders. It might also be nice to adjust the circle radius that way. I think it's time to make a reusable drag handle component. Unlike the first one, 
...
<style>
  ...
  details p { margin: 0; padding: 0 1em; background: hsl(200, 10%, 90%); }
