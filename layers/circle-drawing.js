...
/** This component "explodes" the layers to make them visually separate. */
Vue.component('svg-layers', {
    functional: true,
    render(h, context) {
        let t = context.props.t;
        let wrapped = context.children
            .filter((layer) => layer.tag)
            .map((layer, index) =>
                  h('g', {
                      class: 'layer',
                      key: layer.data.attrs['data-layer'],
                      style: {
                          transform: `translate(2px, ${t*(5-0.5*index)}px) rotateX(${t*50}deg) rotateZ(${t*-20}deg) scale(0.8)`,
                      },
                  }, [
                      h('rect', {
                          class: 'glass',
                          ...
                          attrs: {
                              x: "-0.75", y: "-0.75",
                              width: gridCols + 0.5, height: gridRows + 0.5,
                              fill: `hsl(120 10% ${90 - index*3}%)`,
                              'fill-opacity': "0.15",
                          },
                      }),
                      layer,
...
for (let el of document.querySelectorAll("figure")) {
    new Vue({
        ...
        data: {
            ...
            center: {q: 8, r: 5},
            radius: 3.5,
            show: { // toggle the elements of the diagram
                distances: true,
                circle: true,
                bbox: true,
                measureLine: false,
