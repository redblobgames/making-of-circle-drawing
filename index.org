...
The diagram shows the /output/ of the algorithm, but to explain how an algorithm works, I want diagrams that show the /concepts/ behind the algorithm. The main concept here is /distances/, so I want to make a diagram to show distances.
...
To implement this, I added =<text>= to each grid tile, using Vue's ~{{…}}~ to evaluate a JavaScript expression to set the text:

#+begin_export html
<figure>
  <a-step step="12" :diff="true" restrict="&lt;svg" />
</figure>
#+end_export

I haven't changed the JavaScript at all … yet. I want two diagrams on the page, not only one, so I need to have two =<figure>= tags, and also need to change the JavaScript to instantiate two diagrams.
...
<style>
  ...
  .token.punctuation { color: hsl(300 10% 60%); font-weight: normal; }
